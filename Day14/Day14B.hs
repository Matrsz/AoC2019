...
takeFromStock x = filter isNonzero . map (`minus` x)

addToStock:: Chemical -> [Chemical] -> [Chemical]
addToStock x stock = if any (==x) stock then map (`plus` x) stock else x:stock
...
makeIngredient reactions chemical (B req stock)
    ...
    | otherwise = B ((takeFromStock chemical . foldl (flip addToStock) req) rs) (addToStock (excess p chemical) stock)
    where (R p rs) = sufficientReaction reactions chemical

makeIngredients:: [Reaction] -> [Chemical] -> Balance -> Balance
makeIngredients reactions chemicals balance = foldl (flip (makeIngredient reactions)) balance chemicals
...
updateBalance (B req stock) = foldl (flip updateBalance1) (B req stock) req
...
canMake:: [Reaction] -> Balance -> Bool
canMake reactions balance = notElem (Q 1 "ORE") reqs
    where (B reqs stock) = makeFromOre reactions balance

findMax:: [Reaction] -> Int -> Int -> Int -> Int
findMax reaction ore n inc
    | (not . enoughOre) n = findMax reaction ore (1 + (floor . sqrt . fromIntegral) n) inc
    | enoughOre n && enoughOre (n+inc) = findMax reaction ore (n+inc) inc
    | otherwise = if inc == 1 then n else findMax reaction ore n ((floor . sqrt . fromIntegral) inc)
    where enoughOre x = canMake reaction (B [Q x "FUEL"] [Q ore "ORE"])

main :: IO ()
main = do
    input <- readFile "Day14/Input.txt"
    let reactions = reactionList input
    let costOfFuel = 857266
    let oreSupply = 1000000000000
    let initialGuess = div oreSupply costOfFuel
    print $ findMax reactions oreSupply initialGuess initialGuess